...
const Workspace: React.FC = () => {
  ...
  const imageModels = useMemo(() => { const seen = new Set<string>(); const r: ProviderModel[] = []; const src = selPlatform ? [selPlatform] : providers; src.forEach(p => p.models.forEach(m => { if (m.category === 'image' && !seen.has(m.id)) { seen.add(m.id); r.push(m); } })); return r; }, [providers, selPlatform]);
  const videoModels = useMemo(() => { const seen = new Set<string>(); const r: ProviderModel[] = []; const src = selPlatform ? [selPlatform] : providers; src.forEach(p => p.models.forEach(m => { if (m.category === 'video' && !seen.has(m.id)) { seen.add(m.id); r.push(m); } })); return r; }, [providers, selPlatform]);
  const textModels = useMemo(() => { const seen = new Set<string>(); const r: ProviderModel[] = []; const src = selPlatform ? [selPlatform] : providers; src.forEach(p => p.models.forEach(m => { if (m.category === 'text' && !seen.has(m.id)) { seen.add(m.id); r.push(m); } })); return r; }, [providers, selPlatform]);
  const [customVideoModels, setCustomVideoModels] = useState<Array<{id:string; durations:number[]; qualities:string[]}>>(() => { try { const v = localStorage.getItem('ws_custom_video_models'); return v ? JSON.parse(v) : []; } catch { return []; } });
  ...
  const allVideoModels = useMemo(() => { const seen = new Set<string>(); const r: ProviderModel[] = []; const src = selPlatform ? [selPlatform] : providers; src.forEach(p => p.models.forEach(m => { if (m.category === 'video' && !seen.has(m.id)) { seen.add(m.id); r.push(m); } })); customVideoModels.forEach(c => { if (!seen.has(c.id)) { seen.add(c.id); r.push({ id: c.id, name: c.id, category: 'video' }); } }); return r; }, [providers, selPlatform, customVideoModels]);

  const [taskHistory, setTaskHistory] = useState<TaskHistoryItem[]>(() => loadTaskHistory(projectId || ''));

  const saveCustomVideo = () => { /* ... same ... */ };
    ...
    <div className={styles.workspace}>
      ...
      <Modal title={null} open={modelSettingsOpen} onCancel={() => setModelSettingsOpen(false)} footer={null} width={560} centered className={styles.tplModal}>
        ...
        <div className={styles.tplModalBody}>
          ...
          <div className={styles.tplGroup}><div className={styles.tplGroupTitle}><span className={styles.tplGroupIcon}><PictureOutlined /></span>图片模型</div><Select size="small" value={selImageModel} onChange={setSelImageModel} placeholder={imageModels.length > 0 ? '选择图片模型' : '无图片模型'} allowClear style={{width:'100%'}} options={imageModels.map(m => ({ label: m.id, value: m.id }))} /></div>
          <div className={styles.tplGroup}><div className={styles.tplGroupTitle}><span className={styles.tplGroupIcon}><VideoCameraOutlined /></span>视频模型</div><Select size="small" value={selVideoModel} onChange={(v) => { setSelVideoModel(v); const preset = getVideoPreset(v); if (!preset.durations.includes(videoDuration)) setVideoDuration(preset.durations[0]); if (!preset.qualities.includes(videoQuality)) setVideoQuality(preset.qualities[preset.qualities.length - 1]); }} placeholder={allVideoModels.length > 0 ? '选择视频模型' : '无视频模型'} allowClear style={{width:'100%'}} options={allVideoModels.map(m => ({ label: m.id, value: m.id }))} dropdownRender={menu => (<>{menu}<div style={{borderTop:'1px solid var(--divider-color)',padding:'6px 8px'}}><Button type="link" size="small" icon={<PlusOutlined />} onClick={() => { setModelSettingsOpen(false); setTimeout(() => setCustomVideoOpen(true), 100); }} style={{width:'100%'}}>+ 手动添加视频模型</Button></div></>)} /></div>
          <div className={styles.tplGroup}><div className={styles.tplGroupTitle}><span className={styles.tplGroupIcon}><ThunderboltOutlined /></span>文本模型（推理·AI导演）</div><Select size="small" value={selTextModel} onChange={setSelTextModel} placeholder={textModels.length > 0 ? '选择文本模型' : '无文本模型'} allowClear style={{width:'100%'}} options={textModels.map(m => ({ label: m.id, value: m.id }))} /></div>
        </div>
